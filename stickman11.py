...
class StickMan(FloatLayout):
	...
	def draw(self):
		...
		with self.canvas:
			...
			#Set a different color for the eye
			self.eyeColor = Color(.7,.2,.2,0)
			#eyes
			eyes = Point(points=[
			  hc[0]-h.size[0]/4, hc[1]+h.size[1]/5,
			  hc[0]+h.size[0]/4, hc[1]+h.size[1]/5], pointsize=2)
			Color(*self.color)
			#Center line
			self.axis = CLine(width=width, points=[*h.bottom, hc[0],h.bottom[1]-1.05*h.size[1]], source=self.axsrc)
			...
			self.arm11 = CLine(points=[
			   (p[0], p[1]-h.size[1]*.5),
			   (p[0]-h.size[0]*.7, p[1]-h.size[1]*0.7)], width=width, source=self.armsrc[0][0])
			self.arm1j1.origin = self.arm11.points[:2]
			
			PushMatrix() #For arm1 lower part
			self.arm1j2 = Rotate(1,1,0)
			self.arm12 = CLine(points=[self.arm11.points[-2:],
		   	(p[0]-h.size[0]*1.4,p[1]-h.size[1]*1.5)],
			...
			self.arm21 = CLine(points=[
			   (p[0], p[1]-h.size[1]*.5),
			   (p[0]+h.size[0]*.7, p[1]-h.size[1]*0.7)], width=width, source=self.armsrc[1][0])
			self.arm2j1.origin = self.arm21.points[:2]
			PushMatrix() #For arm2, lower.
			self.arm2j2 = Rotate(1,1,0)
			self.arm22 = CLine(points=[
				self.arm21.points[-2:],
			   (p[0]+h.size[0]*1.4,p[1]-h.size[1]*1.5)],
			...
			self.leg12 = CLine(points=[
				self.leg11.points[-2:],(p[0]-h.size[0]*.9, p[1]-h.size[1]*4.5)], width=width, source=self.legsrc[0][1])
			...
			self.leg22 = CLine(points=(self.leg21.points[-2:],(p[0]+h.size[0]*.9, p[1]-h.size[1]*4.5)), width=width)#self.legsrc[1][1])
